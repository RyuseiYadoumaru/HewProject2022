...
bool GamePlay::World6StageSelectScene::Start()
{
	//オブジェクト生成 初期化
	//背景画像
...
Scene::STATE GamePlay::World6StageSelectScene::Update()
{
	camera->Update();
	...
	if (Input::GetKeyTrigger(PK_ENTER) == true || Input::GetControllerTrigger(XINPUT_GAMEPAD_A) == true)//エンター押すと次のシーンへ移動
	{
		...
		{
		case STAGE_01:
			GameEngine::SceneManager::LoadScene("Stage26");
			break;
		case STAGE_02:
			GameEngine::SceneManager::LoadScene("Stage27");
			break;
		case STAGE_03:
			GameEngine::SceneManager::LoadScene("Stage28");
			break;
		case STAGE_04:
			GameEngine::SceneManager::LoadScene("Stage29");
			break;
		case STAGE_05:
			GameEngine::SceneManager::LoadScene("Stage30");
			break;
		}
	}
...
bool GamePlay::World6StageSelectScene::End()
{
	/*	オブジェクト終了処理	*/
	ObjectEnd();
...
bool GamePlay::World6StageSelectScene::Render()
{
	/****	画面クリア	****/
	ClearDisplay();
...
void GamePlay::World6StageSelectScene::RightMove(STAGE_NUMBER stage)
{
...
void GamePlay::World6StageSelectScene::LeftMove(STAGE_NUMBER stage)
{
